...
class orderLogic
{
    ...
    function getOrders($param)
    {
        ...
        if (!$this->dh->checkConnection()) {
            ...
            // SQL statement
            $sql = $this->dh->db_obj->prepare("SELECT `receipts`.`summe`,`products`.`name`, `receipts`.`datum`, `receipts`.`id`, `receipts`.`strasse`, `receipts`.`plz`, `receipts`.`ort`, `orderlines`.`anzahl`, `orderlines`.`product_id`, `orderlines`.`preis` FROM `receipts` INNER JOIN `orderlines` ON `receipts`.`id` = `orderlines`.`receipt_id` INNER JOIN `products` ON `orderlines`.`product_id` = `products`.`id` WHERE `receipts`.`user_id` = (SELECT `id` FROM `users` WHERE `username` = ?) ORDER BY `receipts`.`datum`, `receipts`.`id` ASC");
